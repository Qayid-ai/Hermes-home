...
`~/.hermes/google_oauth_last_url.txt`. Some installed versions do not support a `--format json` flag; if that flag fails, rerun without it and use the printed URL or the saved text file.

Agent rules for this step:
- Extract the authorization URL from stdout or `~/.hermes/google_oauth_last_url.txt` and send that exact URL to the user as a single line.
