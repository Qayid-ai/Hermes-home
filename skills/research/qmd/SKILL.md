...
qmd search "query" -n 5          # Limit results
...
QMD search results prove that a document is indexed; they do **not** prove the source file still exists. If QMD returns a deleted vault file, verify the path on disk before reporting it as present. When the file is gone but QMD still returns it, call it a stale index record and preserve the returned URI/docid for cleanup. Check current local CLI help/status before running purge/reindex commands; do not invent a cleanup command from memory.

See `references/stale-index-after-deletion.md` for the session pattern.

### Vault ↔ QMD index parity audits

When asked whether QMD matches a vault, compare QMD's configured collection roots against live files first. Do not treat files outside configured collections as missing from QMD unless the user intended those roots to be indexed. Watch for cosmetic URI normalisation: QMD may display `README.md`/`AGENTS.md` as lowercase `readme.md`/`agents.md` while `qmd get` resolves both forms correctly. Verify before reporting stale paths.

See `references/vault-index-parity-audit.md` for the audit workflow and reporting standard.

### Collection-level ignore / exclusion rules

Use collection `ignore:` rules in `~/.config/qmd/index.yml` when a subfolder lives inside an indexed collection root but must not enter that collection. The field is a sibling of `path`, `pattern`, and `context`:
...
In qmd 2.1.0, the CLI passes collection `ignore` patterns to `fast-glob` relative to the collection `path`. After changing config, run `qmd update`; previously indexed matching docs are deactivated during the update pass. Verify with `qmd collection show <name>`, `qmd status`, and `qmd ls <name> | grep <excluded-folder>` returning no active docs. Run `qmd embed` only if status says embeddings are needed.

See `references/collection-ignore-exclusion.md` for the AsturLAB Knowledge Ops example.
