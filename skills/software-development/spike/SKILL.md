...
## Revenue and business validation spikes

When the user asks to "start a business", "make revenue", or prove commercial ability quickly, treat it as a revenue spike. The goal is not a polished company; it is the shortest honest path to a paid test.

Use `references/revenue-spike-playbook.md` for the detailed sequence. Default to a narrow productized service before SaaS: market proof, buyer wedge, lead list, landing page, outreach copy, fulfilment runbook, and local verification. Do not claim revenue until payment or a signed commitment exists. Do not send outreach from the user's identity without explicit approval and a payment/booking path.
